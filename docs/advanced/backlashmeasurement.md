...
* Move the focuser out by twice the amount moved by the autofocus routine (e.g. *Auto Focus Initial Offset Steps* multipled by *Auto Focus Step Size* times 2) and measure HFR (HFR2 at position2)
* Move in by the *Auto Focus Step Size* and easure HFR (HFR2 at position1). If HFR1 and HFR2 are very close, backlash is not cleared yet, and repeat (move in by *Auto Focus Step Size*, measure and replace HFR1 and position1 with the new position and measure). Remember the number of times the measurement is done, to a maximum of 3 times
...
Running the focuser backlash tool is very simple: simply click on the *Measure Backlash* button. This will run the routine automatically. Once it's over, the routine will output a result that can be automatically saved in the focuser options.
