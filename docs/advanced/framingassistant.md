...
Inside **Equipment->Rotator** you have to choose the "Manual Rotator" and connect to it. When this little tool is "connected" the application will consider the rotation of the sequence when you start it. During the centering of the object at sequence start the platesolver will determine the actual rotation, compare it with the desired rotation and then sends a signal to the rotator to turn. As we have the manual rotator enabled a small pop up window will show and it will prompt you to turn the camera by xx degrees until you are within the rotational tolerance (set inside *Options->Plate Solving->Rotation Tolerance*).
