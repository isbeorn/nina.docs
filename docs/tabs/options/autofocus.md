...
The number of attempts the autofocus routine should be retried in case of unsuccessful focusing. Be careful when increasing this, as this can unnecessarily lengthen the AF routine when no focus can be achieved anyways, due to for example clouds. Better to be handled with the [advanced sequencer](../../sequencer/advanced/advanced.md)
